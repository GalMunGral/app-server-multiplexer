...
app = Flask(__name__, static_url_path='', static_folder='respotify/build')
...
@app.route('/')
def index():
  return redirect('/index.html?' + request.query_string.decode())
...
def auth_code_callback():
  ...
  with urlopen(req) as f:
    ...
    return redirect(BASE_URL + '?' + urlencode(params))
